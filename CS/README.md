...
### Index
 ...
 Index를 사용하는 이유는 SQL서버에서 테이블을 만들고 데이터를 추가, 수정, 삭제 할 때 데이터의 레코드는 내부적으로 아무런 순서없이 저장됩니다. 이 때의 데이터 저장영역을 Heap이라고 합니다. Heap에서는 Index가 없는 테이블의 데이터를 찾을 때 무조건 전체 데이터 페이지의 처음 레코드부터 끝 페이지의 마지막 레코드까지 다 읽어서 검색 조건과 비교하게 됩니다. 이런 방식은 테이블 스캔 또는 풀 스캔이라고 합니다. 

 이런 경우 양이 많은 테이블에서 일부분의 데이터만 불러 올 때 풀 스캔을 사용하면 처리 성능이 떨어지게 됩니다. 이때 Index를 사용하면 데이터를 Select할 때 빠르게 찾을 수 있게 됩니다.
